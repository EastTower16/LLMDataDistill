...
ABSL_FLAG(std::string, out_model_path, "md_model.jax",
          "model output path");
...
static std::vector<float> kTargetBuffer(kBatchSize, 0.0);
...
void fillTrainData(const std::vector<TrainEx>& inputData){
  ...
  for (int k = 0; k < kBatchSize; k++) {
      ...
      kTargetBuffer[k]=inputData[k].score;
  }
}
float evalTestDataset(const std::vector<TrainEx>& testData,
  ...
  for (int i = 0; i < nn; i++) {
    ...
    for (int k = start; k < end; k++) {
      ...
      sumDiff += std::abs(resultLiteral.data<float>()[k - start] - ex.score);
    }
  }
  LOG(INFO) << "test avg error: " << sumDiff / testData.size();
  return sumDiff / testData.size();
}
bool saveModel(const std::string& path,const std::vector<const aux::DeviceArray*>& params){
...
int main(int argc, char* argv[]) {
  ...
  float bestLoss=evalTestDataset(testData, batchServing, dev, params, kMaxLen);
  std::string line;
    ...
    while (std::getline(file, line)) {
      ...
      if(trainData.size()==kBatchSize){
        ...
        auto x = *aux::PTree::BufferRN<int>(absl::Span<int>(kTrainBuffer),
                                        {kBatchSize, kMaxLen}, dev);
        auto y = *aux::PTree::BufferRN<float>(absl::Span<float>(kTargetBuffer),
                                        {kBatchSize,1}, dev);

        std::tie(params, opt_state, lossPT) = *Unpack3Tuple(optimizer_step_fn(params, opt_state,x,y));
        step +=1;
        auto resultLiteral = *lossPT.ToArray();
        if(step % 20 ==0){
          LOG(INFO)<<"Step:"<<step<<",Loss: "<<resultLiteral.data<float>()[0];
        }                               
        trainData.clear();
      }
    }
    float loss = evalTestDataset(testData, batchServing, dev, params, kMaxLen);
    if(loss < bestLoss){
      bestLoss =loss;
      ...
      LOG(INFO)<<"new best model"<<bestLoss;
    }
  }
  LOG(INFO)<<"Finished, best loss:"<<bestLoss;
  return 0;
}
